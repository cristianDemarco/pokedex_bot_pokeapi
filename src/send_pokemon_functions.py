...
async def send_pokemon_message(
    ...
    message = await context.bot.send_photo(
        ...
        caption=translate(
            ...
            data={
                ...
                "is_legendary": (
                    TEXTS["IT"]["LEGENDARY_POKEMON_MESSAGE"]
                    if pokemon.is_legendary
                    else ""
                ),
                "is_mythical": (
                    TEXTS["IT"]["MYTHICAL_POKEMON_MESSAGE"]
...
def create_keyboard(pokemon: Pokemon, POKEMON_COUNT: int) -> InlineKeyboardMarkup:
    ...
    if there_is_pokemon_before:
        keyboard.append(
            create_button(
                pokemon=pokemon, text=f"< N°{int(pokemon.id) - 1}", id_changer=-1
            )
        )
...
def create_button(
    pokemon: Pokemon, text: str, id_changer: int, variety_changer: int = 0
